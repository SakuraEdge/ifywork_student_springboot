...
public interface CourseDao {
    ...
    @Select("select * from course where id = (select course_id from class_course where class_id = ${id}) and term = #{term}" +
            "and year = #{year}")
    List<Course> selectCourseByTermAndYear(@Param("id")int id,@Param("term") String term,@Param("year") String year);

    @Select("select * from course where id = (select course_id from class_course where class_id = ${id}) and name like '%${word}%';")
